    ...
    void displayBill(int billNumber)
    {
        ...
        for(int i=0;i<billArrayList.size();i++)
        {
            if(billArrayList.get(i).getBill_Number() == billNumber) {
                ...
                System.out.printf("%-20s%-20s%-20s%-20s\n","Item Number","Item Name" ,"Item Quantity" ,"Item Unit Price","Item sub Total");
                for (int j = 0; j < itemArrayList.size(); j++) {
                    if(itemArrayList.get(j).getBill_Number() == billNumber)
                        System.out.printf("%-20s%-20s%-20s%-20s\n",itemArrayList.get(j).getItem_Number(),itemArrayList.get(j).getItem_Name(),itemArrayList.get(j).getItem_Quantity(), itemArrayList.get(j).getItem_Price() , itemArrayList.get(j).getItem_Subtotal());
                }
            }
        }
    }
